...
class AmazonDataset(object):
    ...
    def load_reviews(self):
        ...
        with open("./data/Amazon_Cellphones/user_product_score_time.txt", "r") as f:  # 打开文件
            user_product_score_time = f.readlines()  # 读取文件
            #print(data)
        for line in user_product_score_time:
            arr = line.split(' ')
            #print(arr)
            user_idx = int(arr[0])
            product_idx = int(arr[1])
            word_indices = [int(float(arr[2]))]
            #[int(i) for i in arr[2].split(' ')]  # list of word idx
            if len(word_indices)>2:
                word_indices = [word_indices[0],word_indices[1]]
...
class AmazonDataLoader(object):
    ...
    def get_batch(self):
        ...
        while len(batch) < 64:#self.batch_size:
            # 1) Sample the word
            ...
            if self.cur_word_i >= len(text_list):
                ...
                user_idx, product_idx, text_list = self.dataset.review.data[review_idx]
                product_knowledge = {pr: getattr(self.dataset, pr).data[product_idx] for pr in self.product_relations}
        #print('prod_knowledge:',product_knowledge)
        #print('batch:')
        #print(batch)
        return batch
