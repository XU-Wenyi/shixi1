...
class BatchKGEnvironment(object):
    ...
    def _get_actions(self, path, done):
        ...
        for r, next_node_id in candidate_acts:
            ...
            #    score = 99999.0
            scores.append(score)
        #print('embeds:',src_embed,'; size',len(src_embed))
        #print('scores:',score)#,'; size:',len(score))
    ...
    def _get_state(self, path):
        ...
        if len(path) == 1:  # initial state
            ...
            #print('len=1,state:',state,'; len:',len(state))
            return state
        ...
        if len(path) == 2:
            state = self.state_gen(user_embed, curr_node_embed, last_node_embed, last_relation_embed, zero_embed,
                                   zero_embed)
            #print('len=2,state:',state,'; len:',len(state))
            return state
        ...
        state = self.state_gen(user_embed, curr_node_embed, last_node_embed, last_relation_embed, older_node_embed,
                               older_relation_embed)
        #print('len>2 <1,state:',state,'; len:',len(state))
        return state
    ...
    def reset(self, uids=None):
        ...
        #print('self._batch_path (concern to batch_act_idx):',self._batch_path,'uids:',uids)
        self._done = False
        ...
        self._batch_curr_state = self._batch_curr_state.reshape((-1,400))
    ...
    def batch_step(self, batch_act_idx):
        ...
        for i in range(len(batch_act_idx)):
            ...
            #print('batch_step-act_idx:',act_idx,'; batch_curr_action:',self._batch_curr_actions[i])
            #print('batch_path:',self._batch_path)
            _, curr_node_type, curr_node_id = self._batch_path[i][-1]
            l = len(self._batch_curr_actions[i])
            #print('relation, next_node_type, next_node_id',relation, next_node_type, next_node_id)
            if l>act_idx:
                relation, next_node_id = self._batch_curr_actions[i][act_idx]
            else:
                relation, next_node_id = self._batch_curr_actions[i][-1]
